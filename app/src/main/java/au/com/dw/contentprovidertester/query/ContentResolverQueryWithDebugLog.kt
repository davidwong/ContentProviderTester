...
class ContentResolverQueryWithDebugLog {
    ...
    {
        try {
            // ignore timings due to debug logging adding extra time
            //val startTime = System.nanoTime()
            var queryResult = query(context, params, { s -> s }, secondaryQueries)
            //val finishTime = System.nanoTime()

            return Result.Success(processResult(queryResult, params, 0))
        } catch (e: Exception)
        {
            Result.Error(e)
        }
...
//            throw SecurityException("TEST")
    ...
    {
        Log.d(debugTag, "Start query with params: " + params.toString())
    ...
    ) : MutableMap<String, Any>
    {
        ...
        for (column in columns) {
            Log.d(debugTag, "  Begin: map row for " + column)
            ...
            if (secondaryQueries.isNotEmpty())
            {
                ...
                if (matchingLookup.isNotEmpty())
                {
                    ...
                    matchingLookup.forEach { secondaryQuery ->
                        ...
                        Log.d(debugTag, "  Begin: secondary lookup for " + column)
                        ...
                        Log.d(debugTag, "  End: secondary lookup for " + column)
                    }
                }
                else
                {
                    Log.d(debugTag, "  No matching secondary lookup")
                }
            }
            Log.d(debugTag, "  End: map row for " + column)
        }
        return rowMap
    }
    ...
    private fun mapField(cursor: Cursor, column: String, nameKey: (columnName: String) -> String): Pair<String, Any>
    {
        ...
        Log.d(debugTag, "    map field for column " + column + "=" + row)
